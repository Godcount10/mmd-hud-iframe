...
interface HyperspeedProps {
  effectOptions?: Partial<Omit<HyperspeedOptions, 'colors'>> & { colors?: Partial<Colors> };
  disabled?: boolean;
  maxDpr?: number;
  holdToBoost?: boolean;
}
...
function resizeRendererToDisplaySize(
  ...
  const needResize = canvas.width !== Math.round(width * dpr) || canvas.height !== Math.round(height * dpr);
  if (needResize) {
...
class App {
  ...
  constructor(container: HTMLElement, options: HyperspeedOptions, maxDpr = 1.5, holdToBoost = true) {
    this.options = options;
  ...
  onVisibilityChange() {
    this.pageVisible = document.visibilityState !== 'hidden';
    if (!this.pageVisible && this.animationFrame !== null) {
      cancelAnimationFrame(this.animationFrame);
      this.animationFrame = null;
      this.clock.stop();
    } else if (this.pageVisible && !this.disposed && this.animationFrame === null) {
      this.clock.start();
  ...
  init() {
    ...
    if (this.holdToBoost) {
      this.container.addEventListener('pointerdown', this.onMouseDown as EventListener);
      this.container.addEventListener('pointerup', this.onMouseUp as EventListener);
      this.container.addEventListener('pointerleave', this.onMouseUp as EventListener);
      this.container.addEventListener('pointercancel', this.onMouseUp as EventListener);
    }
  ...
  dispose() {
    ...
    if (this.container) {
      this.container.removeEventListener('pointerdown', this.onMouseDown as EventListener);
      this.container.removeEventListener('pointerup', this.onMouseUp as EventListener);
      this.container.removeEventListener('pointerleave', this.onMouseUp as EventListener);
      this.container.removeEventListener('pointercancel', this.onMouseUp as EventListener);
  ...
  tick() {
    if (this.disposed || !this.pageVisible) {
      this.animationFrame = null;
      return;
    }

    const containerWidth = Math.max(1, this.container.offsetWidth);
    const containerHeight = Math.max(1, this.container.offsetHeight);
...
const initHyperspeed = async () => {
  ...
  try {
    myApp = new App(container, options, props.maxDpr, props.holdToBoost);
    appRef = myApp;
    await myApp.loadAssets();
    if (generation !== initGeneration || props.disabled || appRef !== myApp) {
      myApp.dispose();
      return;
    }
    myApp.init();
...
watch(
  [() => props.effectOptions, () => props.disabled, () => props.maxDpr, () => props.holdToBoost],
  () => {
    void initHyperspeed();
  },
  { deep: true }
